...
- [ ] **Step 4:** Commit (owner) + deploy de **Parte 2a** (Tasks 1–7) SOLO despues de aplicar `009b`. El cliente nuevo usa `onConflict: 'user_id,athlete_id'`; sin el unique compuesto, el primer insert/reparacion de `athlete_profiles` falla con "no unique or exclusion constraint matching the ON CONFLICT specification".
...
- **Ajustes de review, ronda 5 (2026-07-04):** (1) onboarding preserva `strengthProfile`/`recoveryProfile` existente cuando el paso opcional viene vacio, evitando perder `pullUpMaxReps` cargado desde Ajustes; (2) una op hija de atleta gestionado inexistente localmente se clasifica como validacion no-retriable y se descarta de la cola; (3) el gate de deploy queda explicito: `009a`/`009b` antes del bundle nuevo, `009c` solo despues de confirmar bundle nuevo.
