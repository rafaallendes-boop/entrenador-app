...
function makeSquashSession(date: string, sessionKind: NonNullable<Session['squashDetails']>['sessionKind']): Session {
  return {
    ...
    squashDetails: {
      ...
      drills: [{ name: sessionKind === 'shadows' ? 'Desplazamientos sin pelota a cuatro esquinas' : sessionKind === 'control' ? '100 drops en solitario (50 por lado)' : 'Tiros paralelos profundos', durationMin: 20 }],
    },
  } as Session
}
