...
Suite completa verificada sobre el trabajo sin commitear: **2577/2577 tests**, lint y build OK.
...
- **Fuerza — desacople del nombre, Entregas 1–3** (2026-08-01, **IMPLEMENTADAS Y REVISADAS SIN COMMITEAR**): selector y resolución deterministas; rutas estructurales separadas; `loadReference` unifica lift/factor sin ampliar la elegibilidad del selector; `prescriptionUnit` cubre las cinco planchas; carga derivada fail-closed para nombres sin metadata. Corrige `bodyweight_squat` (82,5 kg → sin carga) y retira `pullUp` del contrato numérico. El code review posterior agregó procedencia `exact`/`alias`/`substring`: un match parcial ya no puede saltarse las salvaguardas de protocolo, implemento, segundos o potencia. El snapshot actual se presenta como contrato post-migración y los 25 factores se congelan en un ledger literal independiente. Una segunda ronda de review agregó `matchKind: 'ambiguous'` con `candidates`: la ambigüedad sigue siendo fail-closed para la carga, pero ya no degrada el bloque a `other` ni inventa un `%1RM`. Verificado: 2577/2577 tests, lint y build OK. Code review cerrado; pendiente solo el commit, sin migraciones.
...
## Reglas del proyecto
- **Fuerza tiene cambios amplios sin commitear en `exerciseLibrary.ts`/`strengthSelector.ts` y consumidores** (ver roadmap §18). La suite está verde; mantener juntas las Entregas 1–3 y no mezclar copy hasta revisar/commitear este bloque.
